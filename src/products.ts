...
export type Order={orderId:`0x${string}`;secret:string;productId:string;createdAt:string;payer?:string;amountUsd?:number;paid?:boolean;txHash?:string;paidAt?:string};
...
export function createOrder(productId:string,payer?:string,amountUsd?:number):Order{const orders=readJson<Order[]>(ordersPath,[]),secret=crypto.randomBytes(32).toString("hex"),orderId=keccak256(stringToHex(secret));const order:Order={orderId,secret,productId,createdAt:new Date().toISOString(),payer,amountUsd,paid:false};orders.push(order);writeJson(ordersPath,orders);return order;}
export function getOrderBySecret(secret:string){return readJson<Order[]>(ordersPath,[]).find(o=>o.secret===secret);}
export function markOrderPaid(secret:string,txHash:Hex|string){const orders=readJson<Order[]>(ordersPath,[]),ix=orders.findIndex(o=>o.secret===secret);if(ix<0)throw new Error("Order not found");orders[ix].paid=true;orders[ix].txHash=String(txHash);orders[ix].paidAt=new Date().toISOString();writeJson(ordersPath,orders);return orders[ix];}
...
export function txAlreadyUsed(txHash:string){return readJson<Sale[]>(salesPath,[]).some(s=>s.txHash.toLowerCase()===txHash.toLowerCase());}
